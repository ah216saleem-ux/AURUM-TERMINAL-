...
import { Activity, Radio, CheckCircle2, Clock, Zap, ArrowUpRight, ArrowDownRight, RefreshCw, Cpu, Server, Wifi } from 'lucide-react';
...
export const LiveTickDebugPanel: React.FC<LiveTickDebugPanelProps> = ({
  ...
  const { selectedMarket, streamStatus, isWebSocketActive, getTickDebug, refreshMarketData } = useMarket();
  ...
  const streamMode = isWebSocketActive ? 'WebSocket' : 'REST FALLBACK';
  const priceSource = isWebSocketActive ? 'LIVE STREAM' : 'REST API';
