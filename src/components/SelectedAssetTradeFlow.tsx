...
${setup.aiReason}`;
  ...
  return (
    <div className="w-full space-y-4 font-sans">
      ...
      {/* Chart Toggle Header */}
      <div className="flex items-center justify-between px-1 font-mono-num">
        <button
          onClick={() => setShowChart(!showChart)}
          className={`py-2 px-3.5 rounded-xl border text-xs font-bold flex items-center gap-2 transition cursor-pointer ${
            showChart
        ...
        >
          ...
          <span>{showChart ? 'HIDE SMC CANDLESTICK CHART' : 'VIEW SMC CANDLESTICK CHART'}</span>
        </button>
      </div>
