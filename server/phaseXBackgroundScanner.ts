...
export async function executePhaseXLiveScanCycle(): Promise<void> {
  ...
  try {
    ...
    const livePrice = verifiedXau?.price && verifiedXau.price > 0 ? verifiedXau.price : 0;
    const now = Date.now();
    ...
    if (gateStatus === 'APPROVED' && (analysis.finalDirection === 'BUY' || analysis.finalDirection === 'SELL')) {
...
export function getPhaseXLiveState(): PhaseXLiveStateResponse {
  ...
  const livePrice = verified?.price || 0;
  const tickAgeSeconds = verified?.ageSeconds ?? 999;
  
  let tickStatus: 'LIVE' | 'LIVE_AMBER' | 'STALE' | 'OFFLINE' = 'OFFLINE';
  if (verified) {
    if (tickAgeSeconds <= 3) {
      tickStatus = 'LIVE';
    } else if (tickAgeSeconds <= 5) {
